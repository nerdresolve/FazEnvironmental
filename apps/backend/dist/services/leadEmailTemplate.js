...
const fg = (color) => `color: ${color} !important;`;
...
function buildInfoRows(lead) {
    const rows = [
        { label: "Cliente", value: lead.name },
        { label: "E-mail", value: lead.email },
    ];
    if (lead.phone)
        rows.push({ label: "Telefone", value: lead.phone });
    if (lead.company)
        rows.push({ label: "Empresa", value: lead.company });
    if (lead.source)
        rows.push({ label: "Origem do contato", value: describeSource(lead.source) });
