/**
 * Provisiona o túnel Cloudflare dedicado e os registros DNS do site FAZ.
 *
 * Idempotente: se o túnel ou os CNAMEs já existirem, reaproveita em vez de
...
const ZONA_NOME = 'fazenvironmental.com.br'
const DOMINIOS = ['fazenvironmental.com.br', 'www.fazenvironmental.com.br']
const TUNEL_NOME = 'faz-main'
...
await cf(`/accounts/${CONTA}/cfd_tunnel/${tunel.id}/configurations`, {
  ...
  body: JSON.stringify({
    config: {
      ingress: [
        // `http://app:3001` é o nome do serviço na rede do compose.
        ...DOMINIOS.map((hostname) => ({ hostname, service: 'http://app:3001' })),
...
console.log(`rota    ${DOMINIOS.join(', ')} -> http://app:3001`)
  ...
  if (jaExiste) {
    ...
    await cf(`/zones/${zona}/dns_records`, {
      ...
      body: JSON.stringify({
        ...
        proxied: true,
        comment: 'Site da FAZ, servido pelo túnel dedicado do compose em infra/',
      }),
    })
